...
class Solution {
    ...
    private int[] dfs(TreeNode root) {
        /* The result is a 2-element array, where values[0] is the max value including the root
        and the values[1] is the max value excluding the root */
    ...
    private int[] calculateValues(TreeNode root, int[] leftSubtree, int[] rightSubtree) {
        ...
        otherwise this would mean robbing two adjacent houses (the values in index 1) */
        int withRoot = root.val + leftSubtree[1] + rightSubtree[1];
        /* When excluding the current node, we sum the max value of the left subtree and the max value of the right subtree,
