class Solution {
    ...
    public int[] topKFrequent(int[] nums, int k) {
        ...
        * Outer loop starts from the last index, which has the group of most frequent elements */
        for (int i = frequencyGroups.length - 1; i > 0; i--) {
            ...
            * the loop just continues */
            for (int n: frequencyGroups[i]) {
                /* Despite the nested loops, this code runs on O(n) because while iterating over the frequencyGroup elements,
                we only need to fill the result array k times, so complexity is O(n + k) and n > k therefore O(n) */
